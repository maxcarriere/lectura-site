/**
 * TTS Monospeaker Demo — lecteur audio interactif pour la page module TTS.
...
(function () {
  ...
  const API_URL = "https://api.lectura.world/tts/synthesize";

  const container = document.querySelector(".tts-demo");
  if (!container) return;
  ...
  async function synthesize() {
    ...
    try {
      if (mode !== "FLUIDE" && window.TTSModes) {
        ...
        setStatus(`Audio genere : ${duration.toFixed(2)}s — Lecture...`);
        ...
        source.onended = function () {
          currentSource = null;
          setStatus(`Lecture terminee (${duration.toFixed(2)}s)`);
          if (progressBar) progressBar.style.width = "0%";
        };
